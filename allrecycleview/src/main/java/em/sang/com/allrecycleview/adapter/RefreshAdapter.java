 ...
 */
public class RefreshAdapter<T> extends DefaultAdapter<T> {

    private int refreshPosition =0;

    public RefreshAdapter(Context context, List lists, int itemID, DefaultAdapterViewLisenter lisenter) {
    ...
    public int getItemViewType(int position) {
        if (position == refreshPosition &&tops.size()>0) {
            return TOP;
        } else if (position < heards.size()+tops.size()) {
            if (position< refreshPosition) {
                return position;
            }else if (position> refreshPosition &&tops.size()>0){
                return position;
            }else {
                return position;
            }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TOP) {
            ...
            if (viewType> refreshPosition &&tops.size()>0) {
